...
  // 默认 null，表示不转换
  let whiteRatio = null;
  if (globals.whiteRatio !== null && globals.whiteRatio !== undefined) {
    // 如果设置了，则解析并限制在 0-100 范围
    whiteRatio = Math.max(0, Math.min(100, parseInt(globals.whiteRatio)));
    if (isNaN(whiteRatio)) {
      whiteRatio = null; // 如果解析失败，也视为不转换
    }
  }
  log("info", `[DEBUG] whiteRatio from globals: ${globals.whiteRatio}`);
  log("info", `[DEBUG] Final whiteRatio: ${whiteRatio === null ? 'Not Set (Skipping color conversion)' : `${whiteRatio}%`}`);
  // ====================
