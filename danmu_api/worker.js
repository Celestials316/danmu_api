  ...
  <main class="main-content">
    ...
    <div class="container">
      ...
      <section id="logs-page" class="page-section">
        <div class="card">
          ...
          <div class="log-controls">
            ...
            <button onclick="toggleAutoRefresh()">
              <span id="autoRefreshText">⏸️ 暂停刷新</span>
            </button>

            <button onclick="clearLogDisplay()">🗑️ 清空显示</button>
            ...
            <div class="log-stats">
              <span>
                ...
                总数: <strong id="logTotal">0</strong>
              </span>
              <span>
                <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor">
                ...
                上限: <strong id="logMax">500</strong>
              </span>
            </div>
          </div>
        ...
        <div class="auto-scroll-indicator" id="autoScrollIndicator" onclick="toggleAutoScroll()" style="display: none;">
          <span class="pulse-dot"></span>
          <span id="autoScrollText">自动滚动</span>
        </div>

        <div class="footer">
          <p>日志每 2 秒自动刷新 | 最多保存 <span id="footerMaxLogs">500</span> 条 | 支持级别筛选和导出</p>
        </div>
      </section>
    </div>
  </main>
  ...
  <script>
    ...
    function startLogRefresh() {
      ...
      refreshInterval = setInterval(async () => {
        if (autoRefreshEnabled && document.getElementById('logs-page').classList.contains('active')) {
          await fetchLogs();
        }
      }, 2000); // 每2秒刷新一次
    ...
    async function fetchLogs() {
      try {
        ...
        const url = window.location.pathname + '/api/logs?format=json' + (level ? '&level=' + level : '');
        ...
        if (data.success) {
          ...
          updateLogStats(data.total, data.maxLogs);
        }
      } catch (error) {
    ...
    // 更新统计信息
    function updateLogStats(total, max) {
      const totalEl = document.getElementById('logTotal');
      const maxEl = document.getElementById('logMax');
      const footerMaxEl = document.getElementById('footerMaxLogs');
      
      if (totalEl) totalEl.textContent = total;
      if (maxEl) maxEl.textContent = max;
      if (footerMaxEl) footerMaxEl.textContent = max;
    }
    ...
    function toggleAutoRefresh() {
      ...
      if (text) {
        text.textContent = autoRefreshEnabled ? '⏸️ 暂停刷新' : '▶️ 开始刷新';
      }
    ...
    function toggleAutoScroll() {
      ...
      if (autoScrollEnabled) {
        ...
        if (text) text.textContent = '自动滚动';
        scrollToBottom();
        showToast('已开启自动滚动');
      } else {
        indicator?.classList.add('paused');
        if (text) text.textContent = '手动滚动';
        showToast('已关闭自动滚动');
      }
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      if (container && indicator) {
        container.addEventListener('scroll', function() {
          ...
          if (!atBottom && container.scrollTop > 100) {
