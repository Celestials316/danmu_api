...
<head>
  ...
  <style>
   ...
   .quick-config-item:hover::before {
     transform: scaleY(1);
   }
         ...
         <div class="card-header">
           ...
           <span class="badge badge-info">
             <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" style="margin-right: 4px;">
               ...
               <path d="M12 16v-4m0-4h0" stroke-width="2" stroke-linecap="round"/>
             </svg>
             常用配置
           </span>
         </div>
...
<div class="quick-config-grid">
  <!-- B站 Cookie -->
  <div class="quick-config-item" onclick="quickEditEnv(&apos;BILIBILI_COOKIE&apos;)">
    <div class="quick-config-icon" style="background: linear-gradient(135deg, #fb7299, #fa5c7c);">
    ...
    <div class="quick-config-content">
      <div class="quick-config-label">B站 Cookie</div>
      <div class="quick-config-value" id="quick-BILIBILI_COOKIE">
        ${globals.envs.BILIBILI_COOKIE ? '已配置' : '未配置'}
      </div>
    </div>
    <div class="quick-config-status">
      ${globals.envs.BILIBILI_COOKIE ? 
        '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" style="color: var(--success);"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" stroke-width="2"/></svg>' : 
        '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" style="color: var(--text-tertiary);"><circle cx="12" cy="12" r="10" stroke-width="2"/></svg>'
      }
    </div>
  </div>

  <!-- TMDB API Key -->
  <div class="quick-config-item" onclick="quickEditEnv(&apos;TMDB_API_KEY&apos;)">
    <div class="quick-config-icon" style="background: linear-gradient(135deg, #01d277, #00b4d8);">
      <svg viewBox="0 0 24 24" width="24" height="24" fill="currentColor">
        <path d="M18.5 2h-13A2.5 2.5 0 0 0 3 4.5v15A2.5 2.5 0 0 0 5.5 22h13a2.5 2.5 0 0 0 2.5-2.5v-15A2.5 2.5 0 0 0 18.5 2zm-10 15H7V7h1.5v10zm5 0h-1.5v-5h1.5v5zm0-7h-1.5V7h1.5v3zm5 7H17v-3h1.5v3zm0-5H17V7h1.5v5z"/>
      </svg>
    </div>
    <div class="quick-config-content">
      <div class="quick-config-label">TMDB API</div>
      <div class="quick-config-value" id="quick-TMDB_API_KEY">
        ${globals.envs.TMDB_API_KEY ? '已配置' : '未配置'}
      </div>
    </div>
    <div class="quick-config-status">
      ${globals.envs.TMDB_API_KEY ? 
        '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" style="color: var(--success);"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" stroke-width="2"/></svg>' : 
        '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" style="color: var(--text-tertiary);"><circle cx="12" cy="12" r="10" stroke-width="2"/></svg>'
      }
    </div>
  ...
  <!-- 代理设置 -->
  <div class="quick-config-item" onclick="quickEditEnv(&apos;PROXY_URL&apos;)">
    <div class="quick-config-icon" style="background: linear-gradient(135deg, #667eea, #764ba2);">
    ...
    <div class="quick-config-content">
      <div class="quick-config-label">代理地址</div>
      <div class="quick-config-value" id="quick-PROXY_URL">
        ${globals.envs.PROXY_URL ? '已配置' : '未配置'}
      </div>
    </div>
    <div class="quick-config-status">
      ${globals.envs.PROXY_URL ? 
        '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" style="color: var(--success);"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" stroke-width="2"/></svg>' : 
        '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" style="color: var(--text-tertiary);"><circle cx="12" cy="12" r="10" stroke-width="2"/></svg>'
      }
    </div>
  ...
  <!-- 弹幕简化 -->
  <div class="quick-config-item" onclick="quickToggleEnv(&apos;DANMU_SIMPLIFIED&apos;)">
    <div class="quick-config-icon" style="background: linear-gradient(135deg, #f093fb, #f5576c);">
      <svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor">
        <path d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z" stroke-width="2"/>
      </svg>
    </div>
    <div class="quick-config-content">
      <div class="quick-config-label">繁简转换</div>
      <div class="quick-config-value" id="quick-DANMU_SIMPLIFIED">
        ${globals.envs.DANMU_SIMPLIFIED ? '已启用' : '已禁用'}
      </div>
    </div>
    <div class="quick-config-status">
      <label class="switch" style="margin: 0;">
        <input type="checkbox" ${globals.envs.DANMU_SIMPLIFIED ? 'checked' : ''} onchange="quickToggleEnv(&apos;DANMU_SIMPLIFIED&apos;, this.checked)">
  ...
  <!-- 限流设置 -->
  <div class="quick-config-item" onclick="quickEditEnv(&apos;RATE_LIMIT_MAX_REQUESTS&apos;)">
    <div class="quick-config-icon" style="background: linear-gradient(135deg, #4facfe, #00f2fe);">
      <svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor">
    ...
    <div class="quick-config-content">
      <div class="quick-config-label">访问限流</div>
      <div class="quick-config-value" id="quick-RATE_LIMIT_MAX_REQUESTS">
        ${globals.rateLimitMaxRequests > 0 ? globals.rateLimitMaxRequests + ' 次/分钟' : '未启用'}
      </div>
    </div>
    <div class="quick-config-status">
      ${globals.rateLimitMaxRequests > 0 ? 
  ...
  <!-- 弹幕数量限制 -->
  <div class="quick-config-item" onclick="quickEditEnv(&apos;DANMU_LIMIT&apos;)">
    <div class="quick-config-icon" style="background: linear-gradient(135deg, #fa709a, #fee140);">
      <svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor">
    ...
    <div class="quick-config-content">
      <div class="quick-config-label">弹幕数量</div>
      <div class="quick-config-value" id="quick-DANMU_LIMIT">
        ${globals.danmuLimit > 0 ? globals.danmuLimit + ' 条' : '不限制'}
      </div>
    </div>
    <div class="quick-config-status">
      <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" style="color: var(--primary-400);">
        ...
        <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" stroke-width="2"/>
         ...
         <div style="margin-top: 24px; padding: 16px; background: rgba(99, 102, 241, 0.05); border: 1px solid rgba(99, 102, 241, 0.2); border-radius: 12px; font-size: 13px; color: var(--text-secondary); line-height: 1.6;">
           <strong style="color: var(--primary-400);">💡 快速配置说明：</strong><br>
           点击配置项即可快速编辑 | 开关类配置可直接切换 | 配置修改后自动保存
         </div>
       </div>
...
// ========== 快速配置功能 ==========
   function quickEditEnv(key) {
     editEnvVar(key);
   }

   async function quickToggleEnv(key, value) {
     if (value === undefined) {
     ...
     // 更新显示
     const quickValue = document.getElementById(`quick-${key}`);
     if (quickValue) {
       quickValue.textContent = value ? '已启用' : '已禁用';
     }
     ...
     try {
       ...
       if (result.success) {
         showToast(\`\${key} 已\${value ? '启用' : '禁用'}\`, 'success');
         updateConfigDisplay(key, value);
       } else {
         throw new Error(result.errorMessage || '保存失败');
       }
