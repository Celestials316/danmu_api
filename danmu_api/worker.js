         ...
         <div class="config-grid">
           <div class="config-item" style="background: var(--bg-primary); border: 2px solid var(--border-color); border-radius: 12px; padding: 20px; position: relative; overflow: hidden;">
             ...
             <div class="form-group" style="margin-bottom: 8px; position: relative; z-index: 1;">
               ...
               <div class="form-hint" style="margin-top: 12px;">请输入接收弹幕的播放器地址。系统会自动在末尾追加 <code style="background:var(--bg-secondary);padding:2px 4px;border-radius:4px;">http://.../comment/id.xml</code> 链接
                 <button class="btn btn-secondary" onclick="showNetworkScanModal()" style="padding: 4px 10px; font-size: 11px; margin-left: 10px; display: inline-flex; align-items: center; gap: 4px;">
...
<!-- 局域网设备扫描弹窗 -->
 <div class="modal-overlay" id="networkScanModal">
   <div class="modal" style="max-width: 700px; max-height: 90vh;">
     <div class="modal-header">
       ...
       <button class="modal-close" onclick="closeModal('networkScanModal')">
         <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor">
           <path d="M6 18L18 6M6 6l12 12" stroke-width="2" stroke-linecap="round"/>
         </svg>
       </button>
     ...
     <div class="modal-body">
       <div class="alert alert-info" style="margin-bottom: 20px;">
         ...
         <span>💡 扫描局域网设备，自动发现播放器地址（预计耗时 5-10 秒）</span>
       </div>

       <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 12px; margin-bottom: 20px;">
         <div class="form-group" style="margin-bottom: 0;">
           <label class="form-label">网段前缀（如：192.168.1）</label>
           <input type="text" class="form-input" id="subnetInput" placeholder="192.168.1" value="192.168.1" style="padding: 10px 12px;">
         </div>
         <div class="form-group" style="margin-bottom: 0;">
           <label class="form-label">扫描范围</label>
           <select class="form-select" id="scanRangeSelect" style="padding: 10px 12px;">
             <option value="fast">快速 (1-50)</option>
             <option value="full" selected>完整 (1-254)</option>
             <option value="custom">自定义</option>
           </select>
         </div>
       </div>

       <div id="customRangeDiv" style="display: none; margin-bottom: 20px;">
         <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
           <div class="form-group" style="margin-bottom: 0;">
             <label class="form-label">起始 IP</label>
             <input type="number" class="form-input" id="startIpInput" placeholder="1" min="1" max="254" value="1" style="padding: 10px 12px;">
           </div>
           <div class="form-group" style="margin-bottom: 0;">
             <label class="form-label">结束 IP</label>
             <input type="number" class="form-input" id="endIpInput" placeholder="254" min="1" max="254" value="254" style="padding: 10px 12px;">
           </div>
         </div>
       </div>

       <div class="form-group">
         <label class="form-label">扫描端口（多个端口用逗号分隔）</label>
         <input type="text" class="form-input" id="portsInput" placeholder="9978,8080,8096,32400" value="9978,8080" style="padding: 10px 12px;">
         <div class="form-hint">常用端口：9978(OK影视), 8080(Kodi/PotPlayer), 8096(Jellyfin), 32400(Plex)</div>
       </div>

       <button class="btn btn-primary" onclick="startNetworkScan()" id="startScanBtn" style="width: 100%; margin-bottom: 20px;">
         <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor">
           <path d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" stroke-width="2"/>
         </svg>
         开始扫描
       ...
       <div id="scanProgress" style="display: none; margin-bottom: 20px;">
         <div style="background: var(--bg-primary); border-radius: 10px; padding: 16px;">
           <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
             <span style="font-size: 13px; color: var(--text-secondary);">扫描进度</span>
             <span id="progressText" style="font-size: 13px; color: var(--primary-400); font-weight: 600;">0%</span>
           </div>
           <div style="height: 8px; background: var(--border-color); border-radius: 10px; overflow: hidden;">
             <div id="progressBar" style="height: 100%; width: 0%; background: linear-gradient(90deg, var(--primary-500), var(--primary-600)); transition: width 0.3s;"></div>
           </div>
         </div>
       </div>

       <div id="scanResults" style="max-height: 300px; overflow-y: auto;">
         <div style="text-align: center; padding: 40px; color: var(--text-tertiary);">
           <svg viewBox="0 0 24 24" width="48" height="48" fill="none" stroke="currentColor" style="opacity: 0.3; margin-bottom: 12px;">
             <path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" stroke-width="2"/>
             <polyline points="9 22 9 12 15 12 15 22" stroke-width="2"/>
           </svg>
           <div>点击上方按钮开始扫描</div>
     ...
     <div class="modal-footer">
       <button class="btn btn-secondary" onclick="closeModal('networkScanModal')">关闭</button>
     </div>
   </div>
 </div>
...
// 🔥 显示局域网扫描弹窗
   function showNetworkScanModal() {
     ...
     // 监听扫描范围变化
     const rangeSelect = document.getElementById('scanRangeSelect');
   ...
   async function startNetworkScan() {
     ...
     const rangeSelect = document.getElementById('scanRangeSelect').value;
     ...
     if (!subnet) {
       showToast('请输入网段前缀', 'error');
       return;
     }
     
     if (!portsInput) {
       showToast('请输入扫描端口', 'error');
       return;
     }
     ...
     if (rangeSelect === 'fast') {
       ...
       startIp = parseInt(document.getElementById('startIpInput').value) || 1;
       endIp = parseInt(document.getElementById('endIpInput').value) || 254;
     }
     
     if (startIp < 1 || startIp > 254 || endIp < 1 || endIp > 254 || startIp > endIp) {
       showToast('IP 范围无效（1-254）', 'error');
       return;
     }
     
     startScanBtn.disabled = true;
     startScanBtn.innerHTML = '<span class="loading-spinner" style="width:14px;height:14px;border-width:2px;"></span> 扫描中...';
     scanProgress.style.display = 'block';
     scanResults.innerHTML = '<div style="text-align: center; padding: 40px; color: var(--text-secondary);"><span class="loading-spinner" style="width:32px;height:32px;border-width:3px;"></span><div style="margin-top: 16px;">正在扫描网络...</div></div>';
     ...
     try {
       ...
       const response = await fetch('/api/network/scan?' + queryParams.toString(), {
         cache: 'no-cache',
       ...
       if (result.success && result.devices && result.devices.length > 0) {
         const devicesHtml = result.devices.map((device, index) => {
           return '<div style="background: var(--bg-primary); border: 1px solid var(--border-color); border-radius: 10px; padding: 14px; margin-bottom: 10px; cursor: pointer; transition: all 0.2s;" onmouseover="this.style.borderColor=\'var(--primary-500)\'; this.style.background=\'var(--bg-hover)\';" onmouseout="this.style.borderColor=\'\'; this.style.background=\'var(--bg-primary)\';" onclick="useScannedDevice(\'' + device.url + '\')">' +
             '<div style="display: flex; justify-content: space-between; align-items: center;">' +
             '<div>' +
             '<div style="font-weight: 600; color: var(--text-primary); margin-bottom: 4px;">' + device.name + '</div>' +
             '<div style="font-size: 12px; color: var(--text-secondary); font-family: monospace;">' + device.ip + ':' + device.port + '</div>' +
             '</div>' +
             '<button class="btn btn-primary" style="padding: 6px 12px; font-size: 12px;">使用此地址</button>' +
             '</div>' +
             '</div>';
         }).join('');
         
         scanResults.innerHTML = '<div style="margin-bottom: 12px; font-size: 13px; color: var(--success); font-weight: 600;">✅ 发现 ' + result.devices.length + ' 个设备（共扫描 ' + result.totalScanned + ' 个地址）</div>' + devicesHtml;
         showToast('✅ 扫描完成，发现 ' + result.devices.length + ' 个设备', 'success');
       } else {
         scanResults.innerHTML = '<div style="text-align: center; padding: 40px; color: var(--text-tertiary);">' +
           '<svg viewBox="0 0 24 24" width="48" height="48" fill="none" stroke="currentColor" style="opacity: 0.3; margin-bottom: 12px;"><path d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" stroke-width="2"/></svg>' +
           '<div>未发现设备</div>' +
           '<div style="font-size: 12px; margin-top: 8px;">尝试调整扫描范围或端口</div>' +
           '</div>';
         showToast('⚠️ 未发现设备', 'warning');
       }
       
     } catch (error) {
       clearInterval(progressInterval);
       console.error('扫描失败:', error);
       scanResults.innerHTML = '<div style="text-align: center; padding: 40px; color: var(--error);">' +
         '<svg viewBox="0 0 24 24" width="48" height="48" fill="none" stroke="currentColor" style="opacity: 0.5; margin-bottom: 12px;"><path d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" stroke-width="2"/></svg>' +
         '<div>扫描失败</div>' +
       ...
       startScanBtn.innerHTML = '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor"><path d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" stroke-width="2"/></svg> 开始扫描';
       setTimeout(() => {
   ...
   // 🔥 使用扫描到的设备地址
   function useScannedDevice(url) {
     const input = document.getElementById('pushTargetUrl');
     if (input) {
       input.value = url;
       localStorage.setItem('danmu_push_url', url);
       showToast('✅ 已选择设备地址', 'success');
       closeModal('networkScanModal');
...
// GET /api/network/scan - 扫描局域网设备
  if (path === "/api/network/scan" && method === "GET") {
    try {
      ...
      const startIp = parseInt(url.searchParams.get('start')) || 1;
      const endIp = parseInt(url.searchParams.get('end')) || 254;
      const portsParam = url.searchParams.get('ports') || '9978,8080';
      
      const ports = portsParam.split(',').map(p => parseInt(p.trim())).filter(p => p > 0 && p < 65536);
      ...
      log("info", "[network/scan] 开始扫描 " + subnet + "." + startIp + "-" + endIp + " 端口: " + ports.join(','));
      ...
      for (let i = startIp; i <= endIp; i++) {
        ...
        for (const port of ports) {
          ...
          scanPromises.push(
            ...
            .then(() => {
              const deviceName = port === 9978 ? 'OK影视' :
                               port === 8080 ? 'Kodi/PotPlayer' :
                               port === 8096 ? 'Jellyfin' :
                               port === 32400 ? 'Plex' : '未知设备';
              
              const basePath = port === 9978 ? '/action?do=refresh&type=danmaku&path=' :
                              port === 8080 ? '/jsonrpc?Player.Open=' :
                              port === 8096 ? '/api/session' :
                              port === 32400 ? '/library/sections' : '/';
              ...
              log("info", "[network/scan] 发现设备: " + ip + ":" + port);
            })
            .catch(() => {})
          );
        }
      ...
      log("info", "[network/scan] 扫描完成，发现 " + devices.length + " 个设备，共扫描 " + totalScans + " 个地址");

      return jsonResponse({
        success: true,
        devices: devices,
        scannedRange: subnet + '.' + startIp + '-' + endIp,
        ports: ports,
